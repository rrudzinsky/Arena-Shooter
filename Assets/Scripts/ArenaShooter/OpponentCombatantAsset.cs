...
{
    internal static class OpponentCombatantAsset
    {
        private const string AssetPath = "Assets/Models/CyberOpponentCombatant.fbx";
        private const string ResourceAssetPath = "Assets/Resources/Models/CyberOpponentCombatant.fbx";
        private const string ResourcePath = "Models/CyberOpponentCombatant";

        public static bool TryBuild(Transform parent, ArenaTheme theme)
        {
            var wrapper = new GameObject("Cyber Opponent Combatant Model");
            wrapper.transform.SetParent(parent, false);
            wrapper.transform.localPosition = new Vector3(0f, -0.85f, 0f);
            wrapper.transform.localRotation = Quaternion.identity;
            wrapper.transform.localScale = Vector3.one;

            var instance = InstantiateModel("Imported Cyber Opponent Combatant Mesh", wrapper.transform);
            if (instance == null)
            {
        ...
        private static Material ResolveThemeMaterial(string rendererName, Material source, ArenaTheme theme)
        {
            ...
            if (key.Contains("cyan") || key.Contains("visor") || key.Contains("sensor"))
            {
                return theme.NeonA;
            }

            if (key.Contains("magenta") || key.Contains("enemy") || key.Contains("strip") || key.Contains("spine") || key.Contains("core"))
            {
                return theme.NeonB;
            }

            if (key.Contains("joint") || key.Contains("rubber") || key.Contains("undersuit") || key.Contains("boot") || key.Contains("hand"))
            {
                return theme.Wall;
            }

            return theme.Opponent;
        }
    }
}
