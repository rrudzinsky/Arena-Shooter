...
{
    internal static class GateDoorAsset
    {
        private const string AssetPath = "Assets/Models/CyberSpawnGateDoor.fbx";
        private const string ResourceAssetPath = "Assets/Resources/Models/CyberSpawnGateDoor.fbx";
        private const string ResourcePath = "Models/CyberSpawnGateDoor";

        public static bool TryBuild(Transform parent, ArenaTheme theme, float gateWidth, float gateHeight)
        {
            var wrapper = new GameObject("Cyber Spawn Gate Door Model");
            wrapper.transform.SetParent(parent, false);
            wrapper.transform.localPosition = Vector3.zero;
            wrapper.transform.localRotation = Quaternion.identity;
            wrapper.transform.localScale = Vector3.one;

            var instance = InstantiateModel("Imported Cyber Spawn Gate Door Mesh", wrapper.transform);
            if (instance == null)
            {
