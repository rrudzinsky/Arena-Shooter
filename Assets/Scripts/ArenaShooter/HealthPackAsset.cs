...
{
    internal static class HealthPackAsset
    {
        private const string AssetPath = "Assets/Models/CyberHealthPack.fbx";
        private const string ResourceAssetPath = "Assets/Resources/Models/CyberHealthPack.fbx";
        private const string ResourcePath = "Models/CyberHealthPack";

        public static bool TryBuildPickupModel(Transform parent, ArenaTheme theme)
        {
            var wrapper = new GameObject("Cyber Health Pack Pickup Model");
            ...
            var instance = InstantiateModel("Imported Cyber Health Pack Mesh", wrapper.transform);
            if (instance == null)
            {
            ...
            DroidRenderSetup.Apply(wrapper, StylizedOutlineCategory.Medical);
            ImportedModelUtility.LogModelInstanceDiagnostics("Health pickup imported model", wrapper);
            return true;
        }
