...
{
    internal static class HealingStationAsset
    {
        private const string AssetPath = "Assets/Models/CyberHealingStation.fbx";
        private const string ResourceAssetPath = "Assets/Resources/Models/CyberHealingStation.fbx";
        private const string ResourcePath = "Models/CyberHealingStation";

        public static bool TryBuild(Transform parent, ArenaTheme theme)
        {
            var wrapper = new GameObject("Cyber Healing Station Model");
            wrapper.transform.SetParent(parent, false);
            wrapper.transform.localPosition = Vector3.zero;
            wrapper.transform.localRotation = Quaternion.identity;
            wrapper.transform.localScale = Vector3.one;

            var instance = InstantiateModel("Imported Cyber Healing Station Mesh", wrapper.transform);
            if (instance == null)
            {
