...
All Out War maps should feel like circular battlefields inside the shield dome. The generated map, safety floor, dome backdrop, boundary ring, spawn arc-bands, resource placement, and runtime NavMesh all derive from one shared arena footprint.
...
- `Assets/Scripts/ArenaShooter/ArenaGenerator.cs` builds the circular room graph, clearings, walls, corridors, open spawn zones, safety floor, and invisible boundary ring.
...
- `warGridRadius` is clamped to a compact range and drives room count, dome radius, safety floor radius, and spawn perimeter radius.

## Invariants

- `ArenaLayout.CircularCenter`, `CircularRadius`, `DomeRadius`, and `PerimeterSpawnRadius` are the shared arena footprint values.
- The visible dome, invisible boundary ring, safety floor disk, and perimeter spawn placement must stay aligned to the same center/radius model.
...
6. Safety floor disk and invisible boundary ring are added.
